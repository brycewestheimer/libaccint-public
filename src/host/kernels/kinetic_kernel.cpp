...
/// @file kinetic_kernel.cpp
/// @brief Kinetic energy integral kernel implementation
///
/// Computes kinetic energy integrals T_uv = -1/2 <u|nabla^2|v> using the
...
/// @brief Compute the 1D kinetic energy integral contribution for one direction
///
/// For Cartesian Gaussian basis function g_b with exponent beta and AM index j:
///   d^2/dx^2 g_b = 4*beta^2 * g_b(j+2) - 2*beta*(2j+1) * g_b(j)
///                   + j*(j-1) * g_b(j-2)
///
/// Therefore the 1D kinetic integral is:
///   T_d(i,j) = -1/2 * [4*beta^2 * S_d(i,j+2)
///                       - 2*beta*(2j+1) * S_d(i,j)
///                       + j*(j-1) * S_d(i,j-2)]
///
/// @param S_d 1D overlap recursion table (built with extended range Lb+2)
/// @param i Angular momentum index for first center
/// @param j Angular momentum index for second center
/// @param beta Exponent of second primitive Gaussian
/// @return 1D kinetic energy integral value
Real kinetic_1d(const std::vector<std::vector<Real>>& S_d,
                int i, int j, Real beta) noexcept {
    // Term 1: 4*beta^2 * S(i, j+2)
    Real val = 4.0 * beta * beta * S_d[i][j + 2];

    // Term 2: -2*beta*(2j+1) * S(i, j)
    val -= 2.0 * beta * static_cast<Real>(2 * j + 1) * S_d[i][j];

    // Term 3: j*(j-1) * S(i, j-2) -- only nonzero when j >= 2
    if (j >= 2) {
        val += static_cast<Real>(j * (j - 1)) * S_d[i][j - 2];
    }
...
void compute_kinetic(const Shell& shell_a, const Shell& shell_b,
    ...
    // Temporary storage for 1D recursion tables (extended to Lb+2)
    ...
    for (Size p = 0; p < shell_a.n_primitives(); ++p) {
        ...
        for (Size q = 0; q < shell_b.n_primitives(); ++q) {
            ...
            // Build EXTENDED 1D overlap recursion tables: need S(i, j+2) for kinetic
            build_1d_overlap(La, Lb + 2, XPA_x, XPB_x, one_over_2zeta, Ix);
            build_1d_overlap(La, Lb + 2, XPA_y, XPB_y, one_over_2zeta, Iy);
            build_1d_overlap(La, Lb + 2, XPA_z, XPB_z, one_over_2zeta, Iz);
            ...
            for (int a_idx = 0; a_idx < na; ++a_idx) {
                ...
                for (int b_idx = 0; b_idx < nb; ++b_idx) {
                    ...
                    const Real Tx = kinetic_1d(Ix, lx_a, lx_b, beta);
                    const Real Ty = kinetic_1d(Iy, ly_a, ly_b, beta);
                    const Real Tz = kinetic_1d(Iz, lz_a, lz_b, beta);

                    // 1D overlap integrals in each direction
                    const Real Sx = Ix[lx_a][lx_b];
                    const Real Sy = Iy[ly_a][ly_b];
                    const Real Sz = Iz[lz_a][lz_b];

                    // Total kinetic: T = Tx*Sy*Sz + Sx*Ty*Sz + Sx*Sy*Tz
                    const Real val = Tx * Sy * Sz
                                   + Sx * Ty * Sz
                                   + Sx * Sy * Tz;
