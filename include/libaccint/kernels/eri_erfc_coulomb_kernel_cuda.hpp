...
/// @file eri_erfc_coulomb_kernel_cuda.hpp
/// @brief CUDA erfc-attenuated Coulomb ERI kernel interface
///
/// GPU-accelerated computation of (ab|erfc(omega*r12)/r12|cd) integrals.
...
/**
 * @brief Launch CUDA kernel for erfc-attenuated Coulomb ERIs
 *
 * Computes (ab|erfc(omega*r12)/r12|cd) = full - erf for all shell quartets.
 *
 ...
 */
void launch_eri_erfc_coulomb_kernel(
...
/**
 * @brief Get output buffer size for erfc-Coulomb ERIs
 *
 * @param quartet Device data for the four ShellSets
 * @return Number of doubles needed in the output buffer
 */
[[nodiscard]] size_t eri_erfc_coulomb_output_size(
    const basis::ShellSetQuartetDeviceData& quartet);
 ...
 * @param stream CUDA stream for kernel launch
 */
void dispatch_eri_erfc_coulomb_kernel(
    const basis::ShellSetQuartetDeviceData& quartet,
    double omega,
    const double* d_boys_coeffs,
    double* d_output,
